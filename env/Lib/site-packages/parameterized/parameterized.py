...
PYTHON_DOESNT_HAVE_FIX_FOR_BPO_40126 = sys.version_info[:3] < (3, 7, 8) or (
    sys.version_info[:2] >= (3, 8) and sys.version_info[:3] < (3, 8, 3)
)
...
MOCK_BACKPORT_DOESNT_HAVE_FIX_FOR_BPO_40126 = (
    _mock_backport is not None and _mock_backport.version_info[0] < 4
)

AVOID_CLEARING_MOCK_PATCHES = (
    PYTHON_DOESNT_HAVE_FIX_FOR_BPO_40126 or MOCK_BACKPORT_DOESNT_HAVE_FIX_FOR_BPO_40126
)
...
def get_class_name_suffix(params_dict):
    ...
    params_vals = (
        params_dict.values() if PY3 else (v for (_, v) in sorted(params_dict.items()))
    )
