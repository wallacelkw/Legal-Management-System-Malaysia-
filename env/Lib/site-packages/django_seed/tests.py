...
class PhoneNumberPerson(models.Model):
    phones = (
        ArrayField(
            base_field=models.CharField(("Phone Number"), max_length=50, unique=True)
        )
...
class SeederTestCase(TestCase):
    ...
    def test_not_covered_fields(self):
        ...
        seeder.add_entity(
            ...
            {
                "json": lambda x: {
                    seeder.faker.domain_name(): {"description": seeder.faker.text()}
                },
            },
        )
        inserted_pks = seeder.execute()
    ...
    def test_locale(self):
        ...
        seeder.execute()
        self.assertTrue(
            all([ad.is_cyrillic(game.title) for game in Game.objects.all()])
        )
    ...
    def test_auto_now_add(self):
        ...
        faker = fake
        seeder = Seeder(faker)
        seeder.add_entity(Game, 10, {"created_at": lambda x: date})
...
class LengthRulesTestCase(TestCase):
    def test_max_length(self):
        ...
        self.assertTrue(
            ...
            "name with length {}, does not respect max length restriction of {}".format(
                len(customer.name), name_max_len
            ),
        )
        ...
        self.assertTrue(
            ...
            "country with length {}, does not respect max length restriction of {}".format(
                len(customer.name), country_max_len
            ),
        )

        self.assertTrue(
            len(customer.address) <= address_max_len,
            "address with length {}, does not respect max length restriction of {}".format(
                len(customer.name), address_max_len
            ),
        )

        self.assertTrue(
            len(customer.comments) <= comments_max_len,
            "comments with length {}, does not respect max length restriction of {}".format(
                len(customer.comments), comments_max_len
            ),
        )
...
class RelationshipTestCase(TestCase):
    def test_one_to_one(self):
        ...
        self.assertEqual(
            Reporter.objects.get(id=result[Reporter][0]).pen.pk, result[Pen][0]
        )
    ...
    def test_many_to_one(self):
        ...
        self.assertEqual(
            Article.objects.get(id=results[Article][0]).reporter.pk,
            results[Reporter][0],
        )
...
class EdgeCaseFieldTestCase(TestCase):
    @skipIf(
        ...
        "Postgres database is not configured, or the tests aren't being run with the `actions` argument.",
    )
...
class Animal(models.Model):
    ...
    first_color = models.SmallIntegerField(choices=COLOR_CHOICES, unique=True)

    second_color = models.BigIntegerField(choices=COLOR_CHOICES)
    ...
    farm = models.IntegerField(choices=FARM_CHOICES)
...
class Choices(TestCase):
    def test_fields(self):
        ...
        self.assertTrue(
            animal_object.first_color in [x[0] for x in Animal.COLOR_CHOICES]
        )
        self.assertTrue(
            animal_object.second_color in [x[0] for x in Animal.COLOR_CHOICES]
        )
