...
class NameGuesser(object):
    ...
    def guess_format(self, name):
        ...
        if name in ("first_name", "firstname", "first"):
            return lambda x: faker.first_name()
        if name in ("last_name", "lastname", "last"):
            return lambda x: faker.last_name()

        if name in ("username", "login", "nickname"):
            return lambda x: faker.user_name()
        if name in ("email", "email_address"):
            return lambda x: faker.email()
        if name in ("phone_number", "phonenumber", "phone"):
            return lambda x: faker.phone_number()
        if name == "address":
            return lambda x: faker.address()
        if name == "city":
            return lambda x: faker.city()
        if name == "streetaddress":
            return lambda x: faker.street_address()
        if name in ("postcode", "zipcode"):
            return lambda x: faker.postcode()
        if name == "state":
            return lambda x: faker.state()
        if name == "country":
            return lambda x: faker.country()
        if name == "title":
            return lambda x: faker.sentence()
        if name in ("body", "summary", "description"):
            return lambda x: faker.text()
...
class FieldTypeGuesser(object):
    ...
    def guess_format(self, field):
        ...
        if isinstance(field, NullBooleanField):
            return lambda x: faker.null_boolean()
        if isinstance(field, PositiveSmallIntegerField):
            return lambda x: provider.rand_small_int(pos=True)
        if isinstance(field, SmallIntegerField):
            return lambda x: provider.rand_small_int()
        if isinstance(field, BigIntegerField):
            return lambda x: provider.rand_big_int()
        if isinstance(field, PositiveIntegerField):
            return lambda x: provider.rand_small_int(pos=True)
        if isinstance(field, IntegerField):
            return lambda x: provider.rand_small_int()
        if isinstance(field, FloatField):
            return lambda x: provider.rand_float()
        if isinstance(field, DecimalField):
            return lambda x: random.random()

        if isinstance(field, URLField):
            return lambda x: faker.uri()
        if isinstance(field, SlugField):
            return lambda x: faker.slug()
        if isinstance(field, IPAddressField) or isinstance(
        ...
        if isinstance(field, CommaSeparatedIntegerField) or (
            ...
            and (validate_comma_separated_integer_list in field.validators)
        ):
            return lambda x: provider.comma_sep_ints()

        if isinstance(field, BinaryField):
            return lambda x: provider.binary()
        if isinstance(field, ImageField):
            return lambda x: provider.file_name()
        ...
        if isinstance(field, CharField):
            return (
                lambda x: faker.text(field.max_length)
        ...
        if isinstance(field, DateField):
            return lambda x: faker.date()
        if isinstance(field, TimeField):
            return lambda x: faker.time()
        ...
        if hasattr(field, "_default_hint"):
            return lambda x: field._default_hint[1]
